...
import { PAGE_TUTORS, PATH_DASHBOARD } from '../../routes/paths';
...
export default function TutorCard({ tutor }) {
  const { displayName, coverImg, jobtitle, email, avatar, location } = tutor;
      
  const { push } = useRouter();

  const handleEditRow = (id) => {
    push(PAGE_TUTORS.tutors.details(paramCase(id)));
  ...
  return (
    <Card sx={{ textAlign: 'center' }}>
      <Box sx={{ position: 'relative' }}>
        ...
        />
        {/* <OverlayStyle /> */}
        <Image src={coverImg} alt={coverImg} ratio="21/9" />
      </Box>
      ...
      <Typography
        ...
        onClick={(e) => {
          e.preventDefault();
          handleEditRow(tutor.displayName);
        }}
      >
        {displayName}
      </Typography>
