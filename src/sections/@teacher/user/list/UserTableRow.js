...
export default function UserTable() {
  ...
  const handleDeleteRow = (id) => {
    const deleteRow = students.filter((row) => row.id !== id);
    setSelected([]);
    setStudents(deleteRow);
  };

  const handleDeleteRows = (selected) => {
    const deleteRows = students.filter((row) => !selected.includes(row.id));
    setSelected([]);
    setStudents(deleteRows);
  };

  const denseHeight = dense ? 52 : 72;

  const filteredUsers = students.filter((student) => {
    const matchesName = student.displayName.toLowerCase().includes(filterName.toLowerCase());
      ...
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <HeaderBreadcrumbs
          ...
          links={[
            ...
            { name: 'Student', href: PATH_TEACHER.student.root },
            { name: 'List' },
          ]}
          action={
            <NextLink href={PATH_TEACHER.student.new} passHref>
        ...
        <Card>
          ...
          >
            {STATUS_OPTIONS.map((tab) => {
              const count = students.filter((user) => (tab === 'all' ? true : user.status === tab)).length;
            ...
            <TableContainer sx={{ minWidth: 800, position: 'relative' }}>
              ...
              <Table size={dense ? 'small' : 'medium'}>
                ...
                <TableBody>
                  ...
                  <TableEmptyRows height={denseHeight} emptyRows={emptyRows(page, rowsPerPage, students.length)} />
