...
mock.onPost('/api/kanban/tasks/update').reply((request) => {
  try {
    const { taskId, update } = JSON.parse(request.data);
    const task = board.tasks.find((_task) => _task.id === taskId);
...
mock.onPost('/api/kanban/tasks/move').reply((request) => {
  try {
    ...
    const sourceList = board.lists.find((list) => list.id === task.listId);
